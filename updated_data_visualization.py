...
def visualize_data(group_division):
        ...
        # Plot histogram for Mean CGPA
        plt.figure(figsize=(10, 6))
        counts, bins, patches = plt.hist(all_mean, bins=10, color='blue', edgecolor='black', alpha=0.7)
        plt.title("Histogram of Mean CGPA per Team")
        ...
        for count, bin_edge in zip(counts, bins):
            if count > 0:  # Only add a label if there's a bar
                plt.text(bin_edge + (bins[1] - bins[0]) / 2, count, int(count), ha='center', va='bottom')

        plt.show()

        # Plot histogram for Standard Deviation
        plt.figure(figsize=(10, 6))
        counts, bins, patches = plt.hist(all_sd, bins=10, color='green', edgecolor='black', alpha=0.7)
        plt.title("Histogram of Standard Deviation of CGPA per Team")
        plt.xlabel("Standard Deviation of CGPA")
        plt.ylabel("Frequency")
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        ...
        plt.figure(figsize=(10, 6))
        counts, bins, patches = plt.hist(compiled_z_score, bins=10, color='red', edgecolor='black', alpha=0.7)
        plt.title("Histogram of Z-Scores for Mean CGPA per Team")
        plt.xlabel("Z-Score")
        plt.ylabel("Frequency")
        plt.grid(axis='y', linestyle='--', alpha=0.7)
