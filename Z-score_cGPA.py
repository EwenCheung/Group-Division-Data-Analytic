    ...
    while len(team) < 5 and (sorted_student[0] or sorted_student[1]):  # add student into team when teams is not full
        ...
        while True:
            if not sorted_student[gender_index]:
                break
            elif index_changes[i] >= 0 and pick_index[i] + index_changes[i] < len(sorted_student[gender_index]) and index_changes[i] <= 5:
                # Check if school is not in the team
                if sorted_student[gender_index][pick_index[i] + index_changes[i]]['School'] not in [student['School'] for student in team]:
                    team.append(sorted_student[gender_index].pop(pick_index[i] + index_changes[i]))
                    break
                else:
                    index_changes[i] += 1
            elif index_changes[i] < 0 and abs(pick_index[i] + index_changes[i]) <= len(sorted_student[gender_index]) and abs(index_changes[i]) <= 5:
                # Check if school is not in the team
                if sorted_student[gender_index][pick_index[i] + index_changes[i]]['School'] not in [student['School'] for student in team]:
                    team.append(sorted_student[gender_index].pop(pick_index[i] + index_changes[i]))
                    break
...
def visualize_data(group_division):
    ...
    for i, x in enumerate(group_division):
        ...
        print(f"The mean CGPA of this TEAM is {ind_mean:.2f}")
        print(f"The variance of this TEAM is {variance:.5f}")
        print(f"The standard deviation of this TEAM is {sd:.5f}")
        print()
    ...
    print(max(compiled_z_score))
    print(min(compiled_z_score))
    
    # Plot histogram for z scores
    plt.figure(figsize=(10, 6))
    n, bins, patches = plt.hist(compiled_z_score, bins=10, edgecolor='black', color='skyblue')
    plt.xlabel('Z Score')
